...
LOG_NO_CHANGE_RUNS="${LOG_NO_CHANGE_RUNS:-0}" # 1 = log every quiet run, 0 = reduce disk writes
...
STATE_FILE="${STATE_DIR}/raidwatch_state.env"
BAYMAP_FILE="${STATE_DIR}/raidwatch_baymap.json"
...
save_state() {
  ...
  if [[ -f "$STATE_FILE" && "${LAST_HASH:-}" == "$new_hash" && "${LAST_PROBLEM:-0}" == "$new_problem" && "${LAST_ALERT_EPOCH:-0}" == "$new_alert_epoch" ]]; then
    return 0
  fi
...
check_btrfs() {
  ...
  for m in "${mnts[@]}"; do
    ...
    if echo "$stats" | awk '{ if (($NF+0) > 0) bad=1 } END { exit (bad ? 1 : 0) }'; then
      :
    else
      problem="1"
    fi
...
T = {
  "de": {
    ...
    "md": "MD RAID",
    "details": "Details",
    "note": "Hinweis: Dieser Report wurde automatisch vom Skript raidwatch.sh v1.2 erzeugt.\nCopyright Roman Glos 2026.",
  ...
  "en": {
    ...
    "md": "MD RAID",
    "details": "Details",
    "note": "Note: This report was automatically generated by raidwatch.sh v1.2.\nCopyright Roman Glos 2026.",
...
def parse_mdadm_detail(mdname):
  ...
  try:
    ...
    # saved info by bay from the array-specific baymap (serial/model/disk)
    saved_by_bay = {}
    arr = baymap_data.get(mdname, {}) if isinstance(baymap_data.get(mdname, {}), dict) else {}
    saved = arr.get("members", {}) if isinstance(arr.get("members", {}), dict) else {}
    ...
    # Determine which physical bays belong to this array.
    # Never merge the global slot inventory here, otherwise multi-array systems
    # (for example md1 on bays 1-7 and md3 on bay 8) could incorrectly show
    # unrelated bays as missing for the degraded array.
    if saved_by_bay:
      bays_to_render = sorted(set(saved_by_bay.keys()) | set(by_bay.keys()))
    elif by_bay:
      lo = min(by_bay.keys())
      hi = max(by_bay.keys())
      bays_to_render = list(range(lo, hi + 1))
    else:
      return info

    filled = []
    for bay in bays_to_render:
      rows = by_bay.get(bay)
      if rows:
        filled.extend(rows)
        continue
      rec = saved_by_bay.get(bay, {})
      # For synthesized missing rows, do not display a historical /dev/sdX path.
      # Device names are volatile after hot-swap/re-add and can otherwise appear
      # duplicated or stale in the report. Bay + model + serial are the stable IDs.
      filled.append({
        "disk": "",
        "bay": str(bay),
...
main_once() {
  ...
  if [[ "$should_send" == "1" && "$MAIL_FORMAT" == "html" ]]; then
